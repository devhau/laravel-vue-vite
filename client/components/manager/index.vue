<template>
    <div class="vh-manager">
        <vh-modal
            ...
            @hide="onHideForm()"
        >
            <errors v-if="showErrors" :errors="errors" />
            <slot
            ...
            ></slot>
            {{
                !$slots["content_confirmDelete"] &&
                $t(optionConfig?.title?.confirmDelete)
            }}
            <template #footer="{}">
                <vh-button
                    size="sm"
        ...
        <vh-modal
            :size="
                optionConfig.formsize(
                    { isNew, data: dataUpdate },
                    this.formsize
                )
            "
            :title="
                isNew
                    ? $t('common.manager.title.new')
                    : $t('common.manager.title.edit')
            "
            :show="showFormUpdate"
            :loading="loading"
            @hide="onHideForm()"
            ...
            >
                <template #field_button_action="{ row, index, start }">
                    <span class="vh-manager__body--action">
                        ...
                        <vh-button
                            v-if="
                                optionConfig.checkButtonDelete({
                                    row,
                                    index,
                                    start,
                                })
                        ...
                        <vh-button
                            v-if="
                                optionConfig.checkButtonUpdate({
                                    row,
                                    index,
                                    start,
                                })
        ...
        <div class="vh-manager__footer">
            <vh-row>
                <vh-col class="col-none" style="width: 105px; flex: none">
                    <vh-select
                        v-model="size_page"
...
export default {
    ...
    computed: {
        page_start() {
            if (this.current_page - 3 > 0) return this.current_page - 3;
            return 1;
        },
        page_end() {
    ...
    methods: {
        ...
        onSearch() {
            ...
            api.getAll({
                ...
                if (rs && rs.OK) {
                    let { data, current_page, from, to, last_page, total } =
                        rs.data;
                    this.source = data;
        ...
        onUpdateData() {
            ...
            if (
                !validate ||
                validate({ data: this.dataUpdate, isNew: this.isNew })
            ) {
                if (this.isNew) {
                    ...
                    api.edit(this.dataUpdate[itemKey], this.dataUpdate).then(
                        (rs) => {
                            if (rs.data.OK == true) {
                                this.onSearch();
                                this.onHideForm();
        ...
        onShowFormUpdate(data, isNew = false) {
            ...
            if (data) {
                let { api, validate, itemKey } = this.optionConfig;
                if (data[itemKey]) {
                    this.dataUpdate = JSON.parse(JSON.stringify({}));
                    this.showFormUpdate = true;
                    this.loading = true;
                    api.get(data[itemKey]).then(({ data: DataItem }) => {
                        this.dataUpdate = JSON.parse(
                            JSON.stringify(DataItem.data)
                        );
                        this.loading = false;
                    });
                } else {
        ...
        clearFilter() {
            if (this.optionConfig.query.sort)
                this.sort =
                    this.optionConfig.query.sort({
                        option: this.optionConfig,
                        app: this,
                    }) ?? {};
            if (this.optionConfig.query.filter)
                this.filter =
                    this.optionConfig.query.filter({
                        option: this.optionConfig,
                        app: this,
                    }) ?? {};
            this.onSearch();
